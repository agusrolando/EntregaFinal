...
export const changePassword = async (req, res) => {
    try { 
      ...
      if(err?.name === "TokenExpiredError") return res.status(403).json({status: "error", error: "El tiempo del token ya se termino"})
      else if(err) return res.json({status: "error", error: err})
  
      if(!newPassword || !confirmation) return res.status(400).json({status: "error", error: "Contraseña nueva?"})
      if(comparePasswords(user, newPassword)) return res.json({status: "error", error: "Contraseña identica"})
      if(newPassword != confirmation) return res.json({status: "error", error: "Son distintas las contraseñas."})
